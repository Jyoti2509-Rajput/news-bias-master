...
def make_bokeh_plot(bias_dict):
    ...
    biases = ["left bias", "left-center bias", "least biased", "right-center bias", "right bias"]
    X = list(range(5))
    ...
    # adjust plot width, height
    scale = 1.5
    p.height = int(250 * scale)
    p.width = int(450 * scale)
...
class BiasChecker:
    ...
    def __init__(self):
        ...
        """
        self.clf = joblib.load("C:/Users/91880/Downloads/news-bias-master/news-bias-master/model.pkl")
        self.tf = joblib.load("C:/Users/91880/Downloads/news-bias-master/news-bias-master/vectorizer.pkl")

    def classify_bias(self, s):
        X = self.tf.transform([s]).todense()
        ypred = [math.e**v for v in self.clf.predict_log_proba(X)[0]]
        biases = ["left bias", "left-center bias", "least biased", "right-center bias", "right bias"]
        bias_dict = dict(zip(biases, ypred))
...
def index():
    ...
    html_p = os.path.join(os.path.dirname(__file__), "templates", "index.html")
